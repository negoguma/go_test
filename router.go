...
func match(pattern, path string) (bool, map[string]string) {
	...
	//패턴과 패스를"/"로 구분한 후 부분 문자열 집합의 개수가 다르다면 false를 반환
...
// 	//http 메서드에 맞는 모든 handlers를 반복하면서 요청 URL에 해당하는 handler를 찾음
...
func (r *router) handler() HandlerFunc {
	return func(c *Context) {
		...
		//요청 URL에 해당하는 handler를 찾지 못하면 NotFound처리
...
func (r *router) HandleFunc(method, pattern string, h HandlerFunc) {
	...
	//http 메소드로 등록된 맵에 URL 패턴과 핸들러 함수 등록
	m[pattern] = h
}
