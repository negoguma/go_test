...
func paresJsonBodyHandler(next HandlerFunc) HandlerFunc {
	return func(c *Context) {
...
func staticHandler(next HandlerFunc) HandlerFunc {
	...
	return func(c *Context) {
		//http메서드가 GET이나 HEAD가 아니면 바로 다음 핸들러 수행
		...
		//file의 내용 전달 (next 핸들러로 제어권을 넘기지 않고 요쳥 처리를 종료함)
